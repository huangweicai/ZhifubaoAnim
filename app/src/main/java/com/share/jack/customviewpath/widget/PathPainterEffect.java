    ...
    public PathPainterEffect(Context context, AttributeSet attrs) {
        ...
        mAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                Log.d("TAG", "getAnimatedValue:"+valueAnimator.getAnimatedValue());
                fraction = (float) valueAnimator.getAnimatedValue();//1---0.5---0
                //mEffect = new DashPathEffect(new float[]{length, length}, fraction * length);

                //高度宽度比例（越小高度越小）、空隙、偏移
                mEffect = new DashPathEffect(new float[]{15, 5}, fraction * length);
